        ...
        with argscope(Conv3D, nl=PReLU.symbolic_function, use_bias=True):
            ...
            for i in range(0, agents):
                ...
                if "Dueling" not in self.method:
                    fc0 = FullyConnected(
                        "fc0_{}".format(i), conv3, 512, activation=tf.nn.relu
                    )
                    fc1 = FullyConnected(
                        "fc1_{}".format(i), fc0, 256, activation=tf.nn.relu
                    )
                    fc2 = FullyConnected(
                        "fc2_{}".format(i), fc1, 128, activation=tf.nn.relu
                    )
                    Q = FullyConnected(
                    ...
                    fc0 = FullyConnected(
                        "fc0V_{}".format(i), conv3, 512, activation=tf.nn.relu
                    )
                    fc1 = FullyConnected(
                        "fc1V_{}".format(i), fc0, 256, activation=tf.nn.relu
                    )
                    fc2 = FullyConnected(
                        "fc2V_{}".format(i), fc1, 128, activation=tf.nn.relu
                    )
                    ...
                    fcA1 = FullyConnected(
                        "fc1V_{}".format(i), fcA0, 256, activation=tf.nn.relu
                    )
                    fcA2 = FullyConnected(
                        "fc2V_{}".format(i), fcA1, 128, activation=tf.nn.relu
                    )
                    A = FullyConnected(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--load",
        help="load model",
        default="/vol/biomedic2/av2514/PycharmProjects/DEVLandmarkDetection/DQN/train_log/Fetal_US_3_agents/model-400000",
    )
    parser.add_argument(
        "--task",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=(
            "/vol/medic01/users/aa16914/projects/tensorpack-medical-gitlab/examples/LandmarkDetection/DQN/data/fetal_brain_us_yuanwei_miccai_2018/test_list.txt",
            "a",
        ),
        help="""Filepath to the text file that comtains list of images.
                                    Each line of this file is a full path to an image scan.
    ...
    parser.add_argument(
        "--agents", help="Number of agents to train together", default=3
    )
    ...
    init_player = MedicalPlayer(
        files_list=args.files, screen_dims=IMAGE_SIZE, task="train", agents=args.agents
    )
